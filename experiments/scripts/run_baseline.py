...
def _make_baseline_root(output_root: Path, engine_name: str) -> Path:
    ...
    baseline_root = output_root / "baselines" / engine_name / timestamp
...
def _write_latest_pointer(output_root: Path, baseline_root: Path) -> Path:
    pointer_path = output_root / "latest" / "baseline.txt"
    pointer_path.parent.mkdir(parents=True, exist_ok=True)
    pointer_path.write_text(str(baseline_root), encoding="utf-8")
    return pointer_path
...
def run_baseline_scenarios(
    *,
    ...
    for config_path in config_paths:
        ...
        print(f"Running baseline scenario '{scenario_name}'...", flush=True)
        ...
        print(
            f"Baseline scenario '{scenario_name}' finished with status={step['status']} "
            f"in {float(step['runtime_seconds']):.2f}s.",
            flush=True,
        )
...
def run_baseline_stability(
    *,
    ...
    print("Running baseline stability evaluation...", flush=True)
    ...
    print(
        f"Baseline stability finished with status={step['status']} "
        f"in {float(step['runtime_seconds']):.2f}s.",
        flush=True,
    )
...
def run_engine_baseline(
    *,
    ...
    baseline_root = _make_baseline_root(output_root, engine_name)
    print(f"Baseline root: {baseline_root}", flush=True)
    print(
        f"Engine={engine_name} | scenarios={len(scenario_config_paths)} | "
    ...
    print("Collecting baseline outputs...", flush=True)
    ...
    print("Baseline manifest written.", flush=True)
    report_path = write_markdown_report(baseline_root)
    latest_path = _write_latest_pointer(output_root, baseline_root)
    print(f"Baseline report written to {report_path}.", flush=True)
    print(f"Latest baseline pointer written to {latest_path}.", flush=True)
    return baseline_root


def main() -> None:
    parser = argparse.ArgumentParser(description="Run a full engine baseline experiment.")
    ...
    output_dir = run_engine_baseline(
    ...
    print(f"Baseline output written to {output_dir}")
