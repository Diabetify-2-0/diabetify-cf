...
def test_build_experiment_engine_returns_dace_adapter() -> None:
    adapter = build_experiment_engine(
        "dace",
        settings=Settings(
            model_path="missing-model.pkl",
    ...
    assert isinstance(adapter, DaceExperimentAdapter)
...
def test_build_experiment_engine_passes_config_to_dace_adapter() -> None:
    adapter = build_experiment_engine(
        ...
        config={"engine_options": {"surrogate_n_estimators": 20, "max_changed_features": 2}},
    )

    assert isinstance(adapter, DaceExperimentAdapter)
    assert adapter.engine.options.surrogate_n_estimators == 20
