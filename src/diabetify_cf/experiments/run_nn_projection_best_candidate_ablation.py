...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the target-gate-only NN full-vs-best-prefix-sparse ablation study."
        )
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        default=str(
            ...
            / "ablation"
            / "nn_projection"
            / "projection_ablation_best_candidate_report.json"
        ),
...
def main() -> None:
    ...
    experiment = NNProjectionBestCandidateAblation(
        engine=engine,
        config=config,
    ...
    print(
        "Mean proximity (full -> best sparse): "
        f"{summary['full_mean_proximity']:.6f} -> "
        f"{summary['sparse_mean_proximity']:.6f}"
    )
    print(
        "Mean changed features (full -> best sparse): "
